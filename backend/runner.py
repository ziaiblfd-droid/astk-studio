        ...
        for root_name in ("metadata", "output"):
            ...
            if root.exists():
                for path in root.rglob("*"):
                    if path.is_file():
                        bundle.write(path, path.relative_to(job_dir).as_posix())
        bundle.writestr("README.txt", "ASTK Studio analysis bundle\nGenerated by the prototype backend.\n")
...
def _run_external(store: JobStore, job_id: str, job_dir: Path) -> None:
    ...
    store.update(job_id, stage="ASTK external runner", progress=15)
    ...
    (job_dir / "output" / "runner.log").write_text(process.stdout + "\n" + process.stderr, encoding="utf-8")
    if process.returncode != 0:
        raise RuntimeError(f"ASTK runner exited with code {process.returncode}")
    if not (job_dir / "output" / "results.json").exists():
        raise RuntimeError("ASTK runner did not create output/results.json")
